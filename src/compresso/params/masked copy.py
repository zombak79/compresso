...
class MaskedParam(nn.Module):
    ...
    def __init__(
        ...
        self.mask = torch.ones(self.weight.shape, dtype=torch.bool).to(self.weight.device)
    ...
    def maskedparam_to_coo(self):
        ...
        return CooSparseParam(indices=indices, values=values, shape=(rows, cols), packed_dim="rows" if self.dim==1 else "cols")
